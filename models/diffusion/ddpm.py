...
from models.diffusion.utils import default, disabled_train
...
class GaussianDiffusion(nn.Module):
    ...
    def apply_cond_lr_encoder(self, img_lr, dates):
        ...
        # print("cond0:", cond[0].shape)
        # print("cond1:", cond[1].shape)
        # print("cond2:", cond[2].shape)

        # cond0: torch.Size([2, 12, 64, 10, 10])
        # cond1: torch.Size([2, 12, 128, 5, 5])
        # cond2: torch.Size([2, 12, 256, 3, 3])
        return cond
    ...
    def forward(
        ...
        txt,
        mtd,
        img_hr_down,  # High-resolution (HR) image
    ...
    def p_losses(
        ...
        x_res = [
            self.img2res(img_hr_down, img_lr_up[:, i])
        ...
        for tps in range(img_lr_up.shape[1]):
            ...
            noise_pred = self.denoise_net(
                ...
                class_labels=None,
                metadata=mtd,
                down_block_additional_residuals=[feat[:, tps] for feat in cond_lr],
                up_block_additional_residuals=cond_hr,
            )
        ...
        if not hparams["use_aux_loss"]:
            ...
            aux_loss = (
                ...
                * pixel_wise_closest_sr_sits_aer_loss(x0_pred, img_hr_down, closest_idx)
                + hparams["grad_px_loss_weight"]
                * grad_pixel_wise_closest_sr_sits_aer_loss(
                    x0_pred, img_hr_down, closest_idx
                )
    ...
    def p_sample(
        ...
        txt,
        mtd,
        cond_lr,  # conditioning information (features from low-resolution image) to guide densoing process
        ...
        if noise_pred is None:
            noise_pred = self.denoise_net(
                ...
                class_labels=None,
                metadata=mtd,
                down_block_additional_residuals=cond_lr,
                up_block_additional_residuals=cond_hr,
            )
        b, *_, device = *x.shape, x.device
    ...
    def sample(
        ...
        txt,
        mtd,
        img_hr_down,  # shape of the output image
        ...
        if not hparams["res"]:
            ...
            img = torch.randn(img_hr_down.shape, device=device)
        ...
        for i in range(img_lr_up.shape[1]):
            ...
            for j in it:
                ...
                img, x_recon = self.p_sample(
                    ...
                    txt,
                    mtd,
                    [feat[:, i] for feat in cond_lr],  # x_e
                    cond_hr,  # for conditioning
                )
                if save_intermediate:
                    img = self.res2img(img, img_lr_up[:, i, :, :])
                    x_recon = self.res2img(x_recon, img_lr_up[:, i, :, :])
        ...
        loss = self.get_loss(sr_pred, img_hr_down)
        ...
        aux_loss = (
            hparams["px_loss_weight"]
            * pixel_wise_closest_sr_sits_aer_loss(img_sr, img_hr_down, closest_idx)
            + hparams["grad_px_loss_weight"]
            * grad_pixel_wise_closest_sr_sits_aer_loss(img_sr, img_hr_down, closest_idx)
            + hparams["temp_grad_mag_loss_weight"]
