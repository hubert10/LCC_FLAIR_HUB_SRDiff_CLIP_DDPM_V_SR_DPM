...
class SRDiffTrainer(Trainer):
    def build_model(self):
        ...
        # Conditioning Network using T-CF
        self.cond_net = TConvFormer(
            ...
            stem_channels=64,
            block_channels=hparams["models"]["t_convformer"]["block_channels"][
                :2
            ],  # [64, 128, 256, 512]
            block_layers=hparams["models"]["t_convformer"]["block_layers"][
                1:
            ],  # [2, 2, 5, 2]
            head_dim=32,
    ...
    def training_step(self, sample):
        img_hr = sample["img_hr"]  # torch.Size([4, 5, 512, 512])
        img_hr_down = sample["img_hr_down"]  # torch.Size([4, 5, 64, 64])
        img_lr = sample["img_lr"]  # torch.Size([4, 2, 3, 10, 10])
        txt = sample["txt"]  # torch.Size([4, 12, 3, 64, 64])
        mtd = sample["mtd"]  # torch.Size([4, 19, 64, 64])
        img_lr_up = sample["img_lr_up"]  # torch.Size([4, 2, 3, 64, 64])
        labels = sample["labels"]  # torch.Size([4, 2, 3, 64, 64])
        labels_sr = sample["labels_sr"]  # torch.Size([4, 2, 3, 64, 64])
        ...
        cls_sits, multi_outputs, aer_outputs = self.model(img_hr, img_sr, dates)
    ...
    def sample_and_test(self, sample):
        ...
        img_sr, final_loss = self.model.gaussian.sample(
            img_hr,  # HR image for conditioning
            txt,
        ...
        _, _, aer_outputs = self.model(img_hr, img_sr, dates)
        ...
        for b in range(img_sr.shape[0]):
            s = self.measure.measure(
                ...
                img_hr_down[b],  # reference HR image
